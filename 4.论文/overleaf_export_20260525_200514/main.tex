...
\includepdf[pages=1,pagecommand={\thispagestyle{empty}}]{assets/cover.pdf}
...
\subsection*{5.1 数据来源说明}

题目附件中提供的数据文件只包括两类。第一类为“数据/附件1：2019--2025 年全国碳排放数据.csv”，该文件记录了2019年至2025年全国层面的碳排放数据，字段包括地区、日期、排放部门和CO$_2$排放量等信息。由于该附件既包含全国总量，也包含分部门排放，本文在全国总量分析中只选取$Sector=Total$的数据，避免将总量与部门排放重复相加；同时，2025年数据截至日期不足全年，因此仅用于说明数据口径，不作为全年值参与同比分析和预测基准。第二类为“数据/附件2：2022 年全国 30 个省份碳排放清单.xlsx”，该文件按省份sheet给出了2022年30个省份的碳排放清单，包含总排放、煤炭、石油、天然气和过程排放等分项信息，是问题一和问题二省级碳排放分析的核心来源。

\subsection*{5.2 外部数据补充}

仅依靠题目附件无法直接得到人口、GDP、产业结构和城镇化率等社会经济变量，而这些变量是计算人均碳排放、碳排放强度以及构建STIRPAT模型所必需的。因此，本文额外从《中国统计年鉴》中整理得到“数据/2022数据.xlsx”和“数据/全国数据.xlsx”。其中，“数据/2022数据.xlsx”包含2022年省级人口、GDP、第二产业增加值和城镇化率等指标，用于与附件2的省级碳排放清单合并；“数据/全国数据.xlsx”包含全国层面人口、GDP、煤炭占比等变量，用于问题三中2024年基准变量设置和情景递推预测。

\subsection*{5.3 数据合并与指标构造}

在省级数据处理过程中，本文首先对《中国统计年鉴》整理得到的省级社会经济数据进行字段统一和数值化处理，将地区名称、人口、GDP、第二产业增加值和城镇化率整理为统一口径；随后对附件2中的各省份sheet逐一读取，剔除说明页，只保留排放清单中的总排放记录，并提取总排放、煤炭、石油、天然气和过程排放等分项数据。在此基础上，按照省份名称将社会经济数据与碳排放清单进行匹配合并，形成2022年省级综合数据表。由于附件2仅给出30个省份的碳排放清单且不包含西藏，合并后最终保留30个省份作为省级横截面分析样本。

在合并后的主表中，进一步计算人均CO$_2$、碳排放强度、人均GDP、第二产业占比、技术效率以及煤炭、石油、天然气和过程排放占比等派生指标。最终“数据/2022省级综合表.xlsx”包含“主表”“省份匹配校验”“列缺失值数量”和“描述性统计”四个sheet。校验结果显示，最终主表行数为30，各关键字段缺失值数量为0，能够满足后续空间差异分析、熵权TOPSIS评价、K-means聚类和STIRPAT回归建模需要。
...
差异显著性方面，2022年30省CO$_2$总量均值为380.2822 Mt，但最大值山东为920.9053 Mt，最小值海南为44.6919 Mt，二者相差约20.61倍；排放总量前8位省份合计占30省总排放的50.72\%，说明排放明显集中在少数重点省份。碳排放强度基尼系数为0.3883，高于总量基尼系数，说明省际排放效率差异更突出。进一步观察图\ref{fig:scatter_pc_ci}可知，人均CO$_2$和碳排放强度并非简单随总量同步变化。资源依赖型省份通常处于图中右上方，表现为人均排放和排放强度同时偏高；部分沿海制造业省份虽然排放总量较大，但人均排放或碳排放强度未必同步最高，说明其减排重点更偏向产业结构和能源效率优化。

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{assets/fig1_scatter_pc_ci.png}
\caption{人均CO$_2$与碳排放强度散点图}
\label{fig:scatter_pc_ci}
\end{figure}

熵权TOPSIS评价结果显示，指标权重前三位为人均GDP、煤炭相关排放占比和城镇化率，权重分别为0.2728、0.2215和0.1717。由图\ref{fig:topsis_rank}可见，TOPSIS得分排名靠前的省份为北京、上海、天津、海南、浙江、福建、重庆、广东，这些省份通常具有较高经济发展水平、较低碳排放强度或较好的能源结构；排名靠后的省份为山西、内蒙古、新疆、河北、宁夏、河南、山东、甘肃，主要受煤炭依赖、排放强度偏高或总量压力较大影响。该结果说明，低碳综合水平并不只由排放总量决定，而是由经济效率、能源结构和发展阶段共同决定。

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{assets/fig1_topsis_rank.png}
\caption{TOPSIS低碳综合得分排名图}
\label{fig:topsis_rank}
\end{figure}

在综合评价基础上，进一步进行K-means聚类。聚类模型中K=3时轮廓系数最高，但K=4能够更清晰地区分资源依赖、工业制造、中等转型和经济发达效率型省份，政策解释性更强。因此本文采用K=4作为最终分类，结果如表\ref{tab:cluster}所示。
...
为进一步解释各类型的内部特征，本文绘制四类省份的标准化均值画像，如图\ref{fig:cluster_radar}所示。资源依赖高排放型在煤炭占比、人均CO$_2$和碳排放强度上明显偏高，说明其减排约束主要来自能源结构；工业制造高排放型在总量和第二产业占比上较突出，说明其重点在工业部门节能降碳；中等转型压力型各指标整体居中，应防止高耗能产业转移造成排放反弹；经济发达效率型在人均GDP和城镇化率上优势明显，同时排放强度较低，适合作为绿色技术、绿色金融和碳市场机制的示范区域。

\begin{figure}[H]
\centering
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{assets/fig1_radar_resource.png}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{assets/fig1_radar_industry.png}
\end{minipage}
...
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{assets/fig1_radar_transition.png}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{assets/fig1_radar_efficiency.png}
\end{minipage}
\caption{K-means四类省份标准化均值画像雷达图}
...
\subsection*{8.5 灵敏度分析}

由于情景预测结果依赖未来人口、经济、能源结构和产业结构变化假设，且STIRPAT系数本身来自有限省级样本估计，因此有必要检验关键参数扰动对预测结论的影响。本文从两个层面进行灵敏度分析：第一，对情景参数进行单因素扰动，观察2045年排放量和达峰年份是否明显变化；第二，对STIRPAT模型系数进行扰动，检验驱动因素弹性估计变化是否会改变主要预测判断。
...
进一步地，图\ref{fig:coef_sensitivity}展示了STIRPAT系数在正负扰动下对2045年排放量的影响。可以看出，煤炭相关排放占比系数和人均GDP系数的扰动对预测结果影响较大，说明能源结构调整速度和经济增长方式是长期排放路径中最关键的不确定来源；人口规模、第二产业占比和城镇化率系数扰动的影响相对较小，表明这些因素虽然参与排放递推，但在本文设定的变化范围内不会单独改变总体达峰判断。因此，问题三的核心结论具有一定稳定性，即强化低碳情景能够显著提前达峰并提高2045年减排幅度，而基准情景下达峰时间偏晚、减排潜力不足。

\begin{figure}[H]
\centering
\includegraphics[width=0.84\textwidth]{assets/fig9_stirpat_coef_sensitivity.png}
\caption{STIRPAT系数灵敏度对2045年排放的影响}
\label{fig:coef_sensitivity}
\end{figure}

\clearpage
\includepdf[pages=1,pagecommand={\thispagestyle{plain}}]{assets/policy_report.pdf}
\clearpage
...
本文计算结果由Python程序统一生成。附录列出主要求解脚本，但已删去生成README、项目汇总说明、gitignore等项目文档的工程性代码，仅保留与数据读取、模型计算、结果检验和图表输出相关的程序。
